...
const listContainer = document.getElementById('announcement_list');
fetch('https://developer1.pythonanywhere.com/main/get-announcements')
...
listContainer.addEventListener('click', (e) => {
    fetch(`https://developer1.pythonanywhere.com/main/get-announcements/${e.target.id}`)
        .then(response => response.json())
        .then(data => {
            console.log(data);
            ...
            if (data) {
                ...
                localStorage.setItem('Data_announcement', JSON.stringify(data));
...
function waitForData() {
    ...
    const data = JSON.parse(localStorage.getItem('Data_announcement'));

    // Agar ma'lumotlar mavjud bo'lsa sahifani yuborish
    if (data) {
        window.location = 'announ_item.html';
    } else {
...
//     } else {
//         // Ma'lumotlar hali kelmasa, kuting va tekshirishni qayta boshlash
//         setTimeout(waitForData, 100); // Masalan, 100 millisekund so'ng qayta tekshirish
//     }
// }
