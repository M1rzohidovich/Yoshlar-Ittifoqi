...
const listContainer = document.getElementById('reception_list');
fetch('https://developer1.pythonanywhere.com/main/get-reception') // Change the API endpoint
...
listContainer.addEventListener('click', (e) => {
    fetch(`https://developer1.pythonanywhere.com/main/get-reception/${e.target.id}`) // Change the API endpoint
        .then(response => response.json())
        .then(data => {
            console.log(data);
            ...
            if (data) {
                ...
                localStorage.setItem('Data_reception', JSON.stringify(data));
...
function waitForData() {
    ...
    const data = JSON.parse(localStorage.getItem('Data_reception'));

    // Agar ma'lumotlar mavjud bo'lsa sahifani yuborish
    if (data) {
        window.location = 'qabulxona_item.html';
    } else {
